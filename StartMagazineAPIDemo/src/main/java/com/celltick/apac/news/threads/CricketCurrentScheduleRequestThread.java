...
public class CricketCurrentScheduleRequestThread extends Thread{
	private static final String TAG = CricketCurrentScheduleRequestThread.class.getSimpleName();
	...
	public CricketCurrentScheduleRequestThread(Handler handler) {
		this.mHandler = handler;
	}
	public CricketCurrentScheduleRequestThread(Handler handler,String start_date,String end_date, String cricket_type) {
		this.mHandler = handler;
		...
		call.enqueue(new Callback() {
			...
			public void onFailure(Request request, IOException e) {
				...
				if (errString.startsWith("Unable to resolve host \"contentapi.celltick.com\"")){
					sendMSG(Constant.CRICKET_ERR_RETURN_NO_CONNECTION,StarNewsApp.getContext().getString(R.string.err_return_no_connection));
				} else if (errString.contains("timed out") || errString.contains("timeout")){
					sendMSG(Constant.CRICKET_ERR_RETURN_TIMEOUT, StarNewsApp.getContext().getString(R.string.err_return_timeout));
				} else {
					sendMSG(Constant.CRICKET_ERR_RETURN_UNKNOWN, StarNewsApp.getContext().getString(R.string.err_return_unknown));
				}
			}
			...
			public void onResponse(Response response) throws IOException {
				...
				mSMString = jsonData;
				sendMSG(Constant.CRICKET_SUCCESS,mSMString);
			}
		});
