...
public class FWCHeaderArticleRequestThread extends Thread{
	private static final String TAG = FWCHeaderArticleRequestThread.class.getSimpleName();
	...
	public FWCHeaderArticleRequestThread(String title, Handler handler) {
		this.mHandler = handler;
		this.mTitle = title;
	}

	public FWCHeaderArticleRequestThread(String title, Handler handler, int requestNum) {
		this.mHandler = handler;
		this.mTitle = title;
		this.mRequestNum = requestNum;
	}

	public FWCHeaderArticleRequestThread(String title, Handler handler, int requestNum, String timeStamp) {
		this.mHandler = handler;
		this.mTitle = title;
		this.mRequestNum = requestNum;
	...
	public void run() {
		...
		if (mRequestNum == 0) {
			URL = ApiUrl.getMagazineBaseURL(mTitle)+"&limit="+Constant.NUM_SM_1ST_REQUEST+"&offset=1";
		} else {
			URL = ApiUrl.getMagazineBaseURL(mTitle)+"&limit="+Constant.NUM_SM_NORMAL_REQUEST +"&offset="+1;
		}
